...
_start:
    ...
    add     x6, x2, x4
    subs    w7, w5, w1
    bls     exception           // условный переход к метке обработки некорректного результата беззнаковой операции
    udiv    x8, x6, x7
    umull   x9, w1, w4
    mul     w10, w2, w3
    adds    x10, x1, x10
    beq     exception           // условный переход к метке обработки некорректного результата беззнаковой операции
    udiv    x11, x9, x10
    add     x12, x8, x11
