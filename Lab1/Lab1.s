...
b:
    ...
    .global handler             // метка обработки результата беззнаковой операции
    .type   handler, %function  // определение метки как функции
    .global exit                // метка выхода из программы
    .type   exit, %function     // определение метки как функции
    .global _start              // метка точки входа программы
    .type   _start, %function   // определение метки как функции
handler:
    mov     x0, #1
    b       exit                // переход к метке выхода из программы
exit:
    mov     x8, #93
    svc     #0
...
_start:
    ...
    add     x6, x2, x4
    subs    w7, w5, w1
    bls     handler             // условный переход к метке обработки результата беззнаковой операции
    udiv    x8, x6, x7
    umull   x9, w1, w4
    mul     w10, w2, w3
    adds    x10, x10, x1
    beq     handler             // условный переход к метке обработки результата беззнаковой операции
    udiv    x11, x9, x10
    add     x12, x8, x11
